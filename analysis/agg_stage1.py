...
OUT='results/mod_stage1'
def parse(f):
    t=open(f,errors='ignore').read()
    a=re.findall(r"'ARI':\s*([0-9.]+)",t);n=re.findall(r"'NMI':\s*([0-9.]+)",t);c=re.findall(r"'CA':\s*\D*([0-9.]+)",t)
...
show('recon','recon_both'); show('feat','feat_mask010'); show('edge','edge_drop05')
