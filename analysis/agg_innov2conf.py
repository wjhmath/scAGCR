...
def load(d,tf=None,rn=None):
    ...
    return c
A={}; A.update(load('results/mod_abl2','wo_feat','base')); A.update(load('results/mod_innov2conf'))
m=collections.defaultdict(dict)
for (t,ds),v in A.items(): m[t][ds]=statistics.mean(v)
dsn=set(m['base'])
print('%-9s%8s%9s%9s   判定'%('cfg','ARI','dvs_base','wins/15'))
print('%-9s%8.3f'%('base',statistics.mean(m['base'].values())))
for t in ['nc05','nc10','proto05']:
    ...
    print('%-9s%8.3f%+9.3f%6d/%d   %s'%(t,a,dl,wins,len(ds),ok))
