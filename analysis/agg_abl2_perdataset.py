...
mean={k:statistics.mean(v) for k,v in cell.items()}
print('=== ARI ===')
print('%-20s'%'dataset'+''.join('%9s'%o.replace('wo_','-') for o in order))
for d in datasets:
    row='%-20s'%d[:20]
    for o in order:
        row+=('%9.3f'%mean[(o,d)]) if (o,d) in mean else '%9s'%'-'
    print(row)
print('\n=== Δ vs full(负=该组件在该数据集上有用)===')
